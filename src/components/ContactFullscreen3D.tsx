...
export default function ContactFullscreen({ assistant, onClose }: ContactFullscreenProps) {
  ...
  const hasWa = rawWa.length >= 8
  const waTarget = rawWa.startsWith('0') ? `62${rawWa.slice(1)}` : rawWa

  const rawIg = (assistant.ig && assistant.ig.trim() !== '') ? assistant.ig.replace('@', '').trim() : ''
  const hasIg = rawIg.length > 0

  const handleOpenWa = () => {
    if (hasWa) {
      window.open(`https://wa.me/${waTarget}`, '_blank')
    } else {
      alert(`Kontak WhatsApp ${assistant.name} belum dicantumkan. Silakan hubungi koordinator atau admin lab.`)
    }
  }
  ...
  const handleOpenIg = () => {
    if (hasIg) {
      ...
      window.open(`https://instagram.com/ical.itpln`, '_blank')
    }
  }
  ...
  const socialButtons = (
    <>
      <button
        ...
        className="flex items-center gap-3 px-6 py-3.5 rounded-full bg-white transition-all duration-300 hover:-translate-y-[2px] shadow-[0_4px_16px_rgba(34,197,94,0.14)] hover:shadow-[0_8px_24px_rgba(34,197,94,0.25)] border border-emerald-100 group cursor-pointer"
        title={hasWa ? `Chat WhatsApp ${assistant.name}` : 'WhatsApp belum dicantumkan'}
      >
        <svg viewBox="0 0 24 24" width="24" height="24" fill="#10b981" className="transition-transform duration-300 group-hover:scale-110 shrink-0">
        ...
        <span className="text-slate-700 font-[600] text-[15px] md:text-[16px] group-hover:text-emerald-700 transition-colors duration-300">
          {hasWa ? 'Chat via WhatsApp' : 'WhatsApp Asisten'}
        </span>
      </button>

      <button
        onClick={handleOpenIg}
        className="flex items-center gap-3 px-6 py-3.5 rounded-full bg-white transition-all duration-300 hover:-translate-y-[2px] shadow-[0_4px_16px_rgba(236,72,153,0.14)] hover:shadow-[0_8px_24px_rgba(236,72,153,0.25)] border border-pink-100 group cursor-pointer"
        title={hasIg ? `Kunjungi Instagram @${rawIg}` : 'Instagram @ical.itpln'}
      >
        ...
        <span className="text-slate-700 font-[600] text-[15px] md:text-[16px] group-hover:text-pink-700 transition-colors duration-300">
          {hasIg ? `@${rawIg}` : 'Instagram @ical.itpln'}
        </span>
      </button>
    </>
  )
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full p-6 flex justify-end items-center z-20">
        <button
          onClick={onClose}
          className="font-inter flex items-center gap-2 px-6 py-2.5 rounded-full text-white font-[600] text-[14px] md:text-[15px] tracking-wide transition-all duration-200 hover:-translate-y-[2px] shadow-md hover:shadow-lg bg-[#537AB8]   hover: hover: cursor-pointer"
        >
      ...
      <div className="relative w-full max-w-6xl mx-auto min-h-screen flex flex-col md:flex-row items-center justify-start md:justify-between px-6 md:px-12 z-10 pt-24 pb-8 md:pt-0 md:pb-0">
        ...
        <div className="shrink-0 flex-1 w-full md:max-w-xl animate-customFadeInUp z-20 flex flex-col justify-center mt-4 md:mt-0">
          <h1 
            className="font-serif-elegant font-[800] text-[42px] sm:text-[48px] md:text-[54px] lg:text-[64px] leading-[1.05] tracking-[-0.02em] mb-3 md:mb-4 text-center md:text-left"
          >
            <span style={{ color: '#1B3258' }}>{assistant.name}</span>
          </h1>
          ...
          <div className="flex items-center justify-center md:justify-start gap-2.5 mb-6 md:mb-8 font-inter">
            {assistant.nim && (
              <span
                className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs sm:text-sm font-semibold tracking-wide shadow-xs"
                style={{
                  background: '#EEF4FB',
                  color: '#2F4D7B',
            ...
            {assistant.role && (
              <span
                className="inline-flex items-center px-3.5 py-1 rounded-full text-xs sm:text-sm font-semibold shadow-xs"
                style={{
                  background: assistant.role === 'Koordinator' ? '#FEF3C7' : '#EEF4FB',
                  color: assistant.role === 'Koordinator' ? '#92400E' : '#2F4D7B',
                  border: `1px solid ${assistant.role === 'Koordinator' ? '#FDE68A' : '#C6DBF2'}`,
                }}
              >
                {assistant.role}
              </span>
            ...
            <div className="relative w-full max-w-[320px] md:max-w-[420px] aspect-[4/5] flex justify-center items-center overflow-visible group">
              ...
              <img
                ...
                style={{ 
                  ...
                  boxShadow: '0 25px 50px -12px rgba(92, 139, 200, 0.25)',
                  objectPosition: 'top'
                }}
              />
            </div>
          ) : (
            <div className="relative w-[280px] h-[340px] md:w-[420px] md:h-[500px] flex items-center justify-center bg-slate-50 rounded-[40px] border-2 border-dashed border-slate-200">
